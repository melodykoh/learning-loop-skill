...
3. Read every active watch-list entry's `Root cause` and `Fix` columns. For each, ask: "Would the same fix close both?" **Then apply the NEGATIVE form (added 2026-08-19): take the candidate entry's OWN prescribed remedy and apply it to this incident — if the failure reproduces under that remedy, it is not an instance of that entry.** Mod 1 tests whether a shared fix EXISTS; this detects when a shared fix exists on paper and demonstrably fails in practice. *(2026-08-19: a threshold call on a safety-edit entry was refused on exactly this test — its remedy, "treat a remediation diff as new code, same tests, both polarities," was fully applied and reproduced the failure exactly. Entry stayed at 1 of 2.)* If yes → **increment the cluster, append this incident as a sub-entry (W_N.x) preserving the specific framing/transcript ref**. If no → new entry with count=1.
...
5. **POLARITY CHECK — before folding, restate the entry's KEY and ask whether this instance runs WITH it or AGAINST it (added v4.8, 2026-08-19).** An instance matching an entry's *topic* but running against its *key* is a **counter-datum**, not an increment: record it on the entry with the count unchanged. This item precedes the fold-by-default rule below deliberately — "when in doubt, fold" otherwise pushes counter-instances into the count of the entry they contradict, which quietly makes a deliberately-at-1-of-2 evidence base undecidable.
6. **When in doubt between fold and new: fold.** Sprawl is the bigger cost. Sub-IDs (W_N.a, W_N.b, …) preserve incident-level traceability inside the cluster so the eventual fix author can trace through every test case.
...
- After user verification, update `watch-list.md` (increment + sub-entry, or new entry). **⚠️ MINT IDs COLLISION-SAFELY (added 2026-08-19).** Sub-entry IDs are hand-minted into a file that CONCURRENT sessions also append to, so a same-day suffix is claimable twice. Use a **session-derived** suffix (`2026-08-19a-spark`), not a bare sequential or date one — deliberately NOT "re-read the file immediately before writing," because unforced re-read discipline is the exact thing that fails. *(2026-08-19: two different mechanisms were both filed as `W7.ae` — second occurrence of this documented collision. The date-stamp convention assumed ONE wrap-up per day; three landed on 2026-08-19. Resolved same day by re-IDing the entry NOTHING referenced — direction decided by tracing inbound references, not by seniority.)* Move escalated entries to the Archived section.
...
| C1 | Caveat #3 conflated two correctly-excluded execs with one
       vendor-miscategorized real founder — adds
...
- ⚠️ **Extract from LIVE POINTERS ONLY — a Fix field or plan-path reference — never by grepping every path in the file (added v4.8, 2026-08-19).** The scoping above is the spec and has been since v4.0; the failure mode is an *implementation* that greps the whole document. A ledger records its own past drift ("prior versions cited X — that file does not exist"), so a whole-file scan matches the correction history and reports a defect the ledger already fixed. **Its false-positive rate therefore RISES the better the ledger is maintained** — it punishes the discipline it depends on. Skip any line containing a correction marker (`path correction`, `no longer resolves`, `prior versions cited`, `drifted`). *(2026-08-19: both hits were the watch-list's own record of past drifts; the live Fix field already carried the correct `_archive/` path.)*
- ⚠️ **The same exclusion is now needed on the PII name sweep** (`~/.claude/reference/public-repo-pii.md`) — that sweep will match this session's own post-mortem, which quotes the leaked names. Any check run over a corpus containing RECORDS OF PAST INSTANCES of the thing being checked has this defect.
