...
- After user verification, update `watch-list.md` (increment + sub-entry, or new entry). **⚠️ MINT IDs COLLISION-SAFELY (added 2026-08-19).** Sub-entry IDs are hand-minted into a file that CONCURRENT sessions also append to, so a same-day suffix is claimable twice. Use a **session-derived** suffix (`2026-08-19a-spark`), not a bare sequential or date one — deliberately NOT "re-read the file immediately before writing," because unforced re-read discipline is the exact thing that fails. *(2026-08-19: two different mechanisms are both filed as `W7.ae` — second occurrence of this documented collision. The date-stamp convention assumed ONE wrap-up per day; three landed on 2026-08-19.)* Move escalated entries to the Archived section.
