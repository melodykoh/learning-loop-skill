...
|---|---|---|
| **v4.8.1** | Aug 19, 2026 | **Dotdir filter — a regression from v4.8's own zsh-safety fix.** Replacing Step 2's shell glob with `find` removed the glob's implicit skip of hidden directories, so triage began reporting `.git` as a live concurrent session. Blast radius verified as cosmetic before severity was assigned: cleanup gates on membership in the consolidated set and rejects any id containing `/`, so a dotdir cannot reach a delete branch. Found by a concurrent session's wrap-up, whose fixture had what mine lacked. Fixed test-first; a session id containing dots but not leading with one still lists correctly. |
| **v4.8** | Aug 19, 2026 | **Sub-agent prompts stop asking for evidence they cannot have; cleanup stops guessing filenames.** **(1) Fabrication vector closed.** `scanner-prompt.md` declared *"You have NO access to the conversation this session ran"* and then, in four places, instructed the agent to cross-reference conversation context, quote conversation excerpts, and fill a **required** output field `**Quote:** "[Relevant quote from conversation]"`. A mandatory field that cannot be filled honestly gets filled by inventing — and scan files are the sole input to session-end consolidation, which reads them blind and cannot tell an invented quote from a real one. All four now name the capture files as the only source and say to omit the field rather than compose one. A fifth site in `consolidation-prompt.md` (the Fact gate's *"verified against conversation evidence?"*) is fixed the same way. Unverifiable scratch lines are now kept and marked `UNCORROBORATED` instead of silently discarded. **(2) Cleanup enumerates instead of hardcoding.** Step 6 deleted a fixed list of five filenames; **all five** real session directories on disk held files it did not cover (`auditor-output.json`, `router-output.json`, `consolidation-final.md`, `ROUTED.md`, `HANDOFF-*.md`), so `rmdir` failed, the directory survived, and the gate read it as a silent skip and retried a block that could not succeed. It now enumerates what is actually present and removes each file individually — per-file accountability preserved, never `rm -r`, and a subdirectory makes it stop and report rather than recurse. The unsubstituted placeholder `[consolidated-session-id]` was also a live bash **bracket glob**, not an error; it now HALTs. **(3) Push verification resolves its target.** Step 8 checked `origin/main...main` literally, so on any feature branch or fork it returned `0 0` — a clean pass that proved nothing. Now resolves `@{u}`, and a missing upstream is the finding rather than a pass. **(4) Mod 9 and Mod 10 no longer gated by cluster volume.** *"Skip this step entirely if ≤15 entries"* sat above Mods 5-10, carrying away the graduation monitoring and the path-existence check on almost every wrap-up. Path drift has no relationship to sprawl. |
| **v4.7** | Aug 19, 2026 | **Two routing gates, plus an honest negative result.** **(1) Failure-class gate.** Every conclusion now carries a required `Failure class:` field — `placement` / `shape` / `no-moment` / `n/a` — answered *before* a remedy is chosen, with a mechanical presence check at the top of Step 5 that HALTs if any conclusion lacks one (it counts on `**Zone:**`, not on heading shape — heading shape varies across real runs and a heading-count check misread a 14-conclusion file as 25). `placement` and `shape` are prose problems with working prose fixes. A `no-moment` finding — right place, right words, context-injected, matching case, still silent — may route only to a mechanism, a verification step whose negative result changes the outcome, a test, or an explicit `HUMAN-CAUGHT:` record naming the question that catches it. Never another paragraph. Mod 6 decides *when* to codify; the class decides *what form* a codification may take. **Tested, and the headline premise did not hold:** across two fixtures and six independent pre-change runs — 39 routing decisions, including ten conclusions under explicit end-of-session time pressure — not one chose "a new paragraph" for a `no-moment` finding. What was absent in *every* run was any **recorded** class, and runs silently diverged on remedy class for identical input. The field therefore ships as an **auditability** mechanism on that evidence, not as a correction to observed behaviour, and the skill says so in its own provenance. **(2) Fourth state — in flight elsewhere.** *Consolidated findings + zero inbound references* is produced identically by an abandoned run and by a run currently live in a parallel session. Step 2 triage and Step 6a-ii now test whether any file in a directory was written within 120 minutes; if so the directory is a **state to report, never a decision to surface** — not included, not held, not deleted, not read. The end-of-wrap-up cleanup gate gains the same fourth state, keyed on **ownership** (is this a directory this wrap-up consolidated?) rather than on recency. Ownership is membership in the consolidated set — this session *plus any the user opted in at triage* — never equality with the current session. An unreadable directory is treated the same way, because a swallowed `find` error otherwise reads as "nobody owns this." The `RETAINED:` record gains an owner field and may only be written by the session that consolidated the directory — the dangerous branch here was the one that looked safe. Pre-v4.7 retention records stay valid. **Also:** a standing **gate-state enumeration** checklist — three gates in this skill have now been found missing a branch for a legitimate case. It lives in the repo's `CLAUDE.md` (it fires when someone *edits* the skill, never when it runs), with a one-line pointer row in SKILL.md's Design Principles. |
| **v4.6** | Aug 11, 2026 | **Zone criteria re-keyed onto WHO CAN ANSWER.** The three-tier machinery was right; its criteria were on the wrong axis. All six original Zone-1 triggers described the *edit* or the skill's own internals — *persona challenged · Step 5.6 borderline · new top-level cluster · root CLAUDE.md edit · plan amendment · cross-repo edit* — so not one asked whether the user could answer. **Zone 1** is now *only the user can answer*: publishability under their name · scope crossing onto a shared or external surface · irreversible-or-intolerable-if-wrong · a preference with no defensible default. **Zone 2** is *notify and confirm*, and explicitly keeps items with **no action attached** when they change what the user should expect from the model, harness or workflow. **Zone 3** is *decided* — mechanism calls, logged in the routing record rather than surfaced. The sorting question, *"if the user says 'you decide,' can I?"*, is a required output field answered **before** the zone, so the zone is derived rather than asserted. Persona challenges and Step-5.6 borderlines are **adjudicated and re-zoned**, never auto-promoted, with every resolution stated so ruling on your own work is never silent. Presentation, the Verification Detail Floor and the persona panel are unchanged. Also fixed: SKILL.md pointed at `references/CHANGELOG.md` as version history (abandoned after v4.3) and referenced a "Step 6.5" that lives in the consolidation prompt, not SKILL.md — both now name the real file. |
| v4.5 | Jul 30, 2026 | Step 6a cleanup guard hardened in three ways: the inbound-reference search widened from a hand-written three-surface list to everywhere referrers actually live (with file-existence assertions, and no swallowed errors — a renamed surface used to read as "no references"); a NEW routing-completion check that HALTS on an abandoned run, i.e. a directory holding consolidated findings that nothing references, which the reference check structurally cannot catch; and a third gate state for a **deliberately retained** directory, so a reasoned retention is no longer indistinguishable from a silently-skipped cleanup. |
| v4.4 | Jul 30, 2026 | Sub-agent briefs no longer claim inherited conversation context — both dispatch prompts now state that the agent inherits nothing, name the hand-off file as the mechanism, and require naming a thin hand-off rather than filling the gap. Dispatch tool renamed `Task` -> `Agent` (harness v2.1.63) in `allowed-tools` and the Step-2 spawn line. Added the fan-out fact-sentence: invoking a skill whose documented behavior is dispatch IS the request for it. |
| v4.3 | Jul 26, 2026 | **Step 1b.5 program-concluded exit branch.** The Phase-1 eval trigger (`count ≥ 3` OR `≥7 days since ship`, AND latest decision not <24h old) becomes *permanently* satisfiable once the tracked program closes — all three conditions true forever, with no open decision for the report to inform. A 6th branch now checks the decision log first and suppresses the trigger with a one-line log when the program is recorded closed/retired. Surfaced when a wrap-up skipped the step by judgment and announced the skip; a silent skip would have kept the gap invisible. Companion watch-list entry tracks destination-back-reasoning at mandated steps outside Step 3/3a. |
